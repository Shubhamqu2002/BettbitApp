...
import '../../services/withdrawl/passimpay_withdraw_service.dart';

class PassimpayWithdrawPage extends StatefulWidget {
  ...
  @override
  State<PassimpayWithdrawPage> createState() => _PassimpayWithdrawPageState();
}

class _PassimpayWithdrawPageState extends State<PassimpayWithdrawPage>
  ...
  final PassimpayWithdrawService _service = PassimpayWithdrawService();
  ...
  Future<void> _loadPrefs() async {
    ...
    String country = (prefs.getString('registered_country') ?? 'IN').trim();
    String userName = (prefs.getString('user_name') ?? '').trim();
    String walletId = (prefs.getString('gamer_id') ?? '').trim();

    if (currency.isEmpty) currency = "INR";
    if (country.isEmpty) country = "IN";

    final cc = _getCallingCode(country);
    ...
    debugPrint(
      "🧩 [PASSIMPAY_WITHDRAW_PAGE] prefs => user_name=$_userName | wallet_id=$_walletId | currency=$_currency | country=$_country | calling=$_callingCode",
    );
  }

  String _getCallingCode(String countryCode) {
  ...
  void _validateAll() {
    final walletAddress = _walletAddressCtrl.text.trim();
    ...
    setState(() {
      ...
      if (walletAddress.isEmpty) {
        _walletAddressError = "Crypto wallet address is required";
      } else if (walletAddress.length < 6) {
        _walletAddressError = "Enter valid crypto wallet address";
      }
  ...
  bool get _isValid =>
      ...
      _phoneError == null &&
      _txnPassError == null &&
      _walletAddressCtrl.text.trim().isNotEmpty &&
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final fullPhone = "$_callingCode$phoneDigits";

      debugPrint(
        "🚀 [PASSIMPAY_WITHDRAW_PAGE] HIT API => methodCode=${widget.methodCode} | amount=$amount | orderId=$orderId | symbol=$_symbol | currency=$_currency | user_name=$_userName | wallet_id=$_walletId | phone=$fullPhone",
      );

      final resp = await _service.createPassimpayWithdraw(
        ...
        currency: _currency,
        walletId: _walletId,
        destinationWalletAddress: _walletAddressCtrl.text.trim(),
      ...
      final msg = (resp["message"] ?? "Passimpay withdrawal request submitted")
          .toString();
        ...
        SnackBar(
          content: Row(
            children: const [
              ...
              Expanded(
                child: Text(
                  "Passimpay withdrawal request submitted",
                ...
                child: SlideTransition(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        _HeaderCard(
                          title: "Passimpay Withdraw",
                          subtitle:
                              "Withdraw securely using Passimpay wallet details",
                        ...
                        _InputField(
                          ...
                          hint: "Enter destination wallet address",
                        ...
                        const _SecurityBadge(
                          text: "Your transaction password is AES encrypted",
                        ),
                      ],
                    ),
                  ),
          ...
          Row(
            ...
            children: [
              Icon(Icons.info_outline_rounded,
                  color: warnAccent.withOpacity(0.9), size: 20),
